...
import type { PerfilPedidoDetalhe, PerfilPedidoStatusFluxo } from "../../../types/perfil";
...
const PROXIMO_STATUS_LABEL: Record<PerfilPedidoStatusFluxo, string | null> = {
  "em-separacao": "Pronto",
  pronto: "Enviado",
  enviado: "Finalizado",
  finalizado: null,
...
export function ModalPedidoVenda({
  ...
  const proximoStatusLabel = pedido ? PROXIMO_STATUS_LABEL[pedido.statusFluxoKey] : null;
  const podeAvancarStatus = Boolean(proximoStatusLabel);
  const podeCancelarPedido = pedido?.statusFluxoKey !== "cancelado";
        ...
        <div className="space-y-5">
          <div className="grid gap-3 sm:grid-cols-3">
            <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
              <p className="text-xs uppercase tracking-[0.2em] text-neutral-500">Status atual</p>
            ...
            <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
              <p className="text-xs uppercase tracking-[0.2em] text-neutral-500">Total da venda</p>
          ...
          <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
            ...
            <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-semibold text-white">Descricao do pedido</h3>
              </div>

              <div className="mt-4 space-y-3 text-sm text-neutral-300">
                <p>{pedido.observacao}</p>
                {pedido.motivoCancelamento ? (
                  <div className="rounded-2xl border border-red-400/20 bg-red-400/10 p-3 text-red-100">
                    <p className="text-xs uppercase tracking-[0.2em] text-red-200">
                      Motivo do cancelamento
                    </p>
                    <p className="mt-2">{pedido.motivoCancelamento}</p>
                  </div>
                ) : null}
              </div>
            </div>
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <p className="mt-1 text-sm text-neutral-300">
                    O fluxo abaixo ja esta pronto na interface. Falta apenas integrar a mudanca de
                    status e o cancelamento a um endpoint da API da loja.
                  </p>
                </div>
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  {podeAvancarStatus ? (
                    <Botao
                      ...
                      className="sm:px-4"
                    >
                      Mover para {proximoStatusLabel}
                    </Botao>
                  ) : (
                    <div className="rounded-2xl border border-white/10 bg-black/30 px-4 py-3 text-sm text-neutral-300">
                      {pedido.statusFluxoKey === "cancelado"
                        ? "Pedido encerrado como cancelado."
                        : "Pedido ja esta na ultima etapa do fluxo."}
                    </div>
                  )}
